...
def mapping_fn(my_idml):
    ...
    try:
        ...
        s3.download_file(Bucket=bucket_name, Key=file_key, Filename = '/tmp/layout.json')
...
def lambda_handler(event, context):
    ...
    try:
        s3.download_file(Bucket=bucket_name, Key=file_key, Filename = '/tmp/layout_file.idml')
    ...
    try:
        s3.upload_file("/tmp/layout_file.idml", '', 'Output_idml.idml')
